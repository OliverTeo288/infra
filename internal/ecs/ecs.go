...
func GetECSClusters(profile string) ([]string, error) {
	cmd := exec.Command("aws", "ecs", "list-clusters", "--query", "clusterArns", "--output", "text", "--profile", profile)
...
func SelectECSCluster(profile string) (string, error) {
	clusters, err := GetECSClusters(profile)
	if err != nil {
		return "", err
	}
...
func GetECSServices(cluster, profile string) ([]string, error) {
	cmd := exec.Command("aws", "ecs", "list-services", "--cluster", cluster, "--query", "serviceArns", "--output", "text", "--profile", profile)
	output, err := cmd.Output()
	if err != nil {
...
func SelectECSService(cluster, profile string) (string, error) {
	services, err := GetECSServices(cluster, profile)
	if err != nil {
		return "", err
	}
...
func GetECSTasks(cluster, service, profile string) ([]string, error) {
	cmd := exec.Command("aws", "ecs", "list-tasks", "--cluster", cluster, "--service-name", service, "--query", "taskArns", "--output", "text", "--profile", profile)
	output, err := cmd.Output()
	if err != nil {
...
func SelectECSTask(cluster, service, profile string) (string, error) {
	tasks, err := GetECSTasks(cluster, service, profile)
	if err != nil {
		return "", err
	}
...
func GetTaskDetails(cluster, taskID, profile string) (string, error) {
	cmd := exec.Command("aws", "ecs", "describe-tasks", "--cluster", cluster, "--tasks", taskID, "--query", "tasks[0].containers[0].runtimeId", "--output", "text", "--profile", profile)
	output, err := cmd.Output()
	if err != nil {
...
func GetECSContainers(cluster, taskID, profile string) ([]string, error) {
	cmd := exec.Command("aws", "ecs", "describe-tasks", "--cluster", cluster, "--tasks", taskID, "--query", "tasks[0].containers[].name", "--output", "text", "--profile", profile)
	output, err := cmd.Output()
	if err != nil {
...
func SelectECSContainer(cluster, taskID, profile string) (string, error) {
	containers, err := GetECSContainers(cluster, taskID, profile)
	if err != nil {
		return "", err
	}
...
func StartSSMSession(profile, cluster, taskID, runtimeID, dbHost string) error {
